...
app.get(
	"/",
	function(req, res){
		res.sendfile(__dirname + '/indexUnified.html');
	}
);
			...
			function(err, collection){
				collection.findOne(
					function(err, itemFound){
						...
						getCollectionAsArrayAndSendToResponse("test", {}, res, {sessionId:itemFound.currentSessionId});
					}
				);
			}
		);
...
app.get(
	...
	function(req, res){
		doStuffOnCollection(
			...
			function(err, collection){
				collection.findOne(
					function(err, discussionModeObjectFound){
						console.log("discussionModeObjectFound was " + discussionModeObjectFound);
						if(discussionModeObjectFound==null){
							discussionModeObjectFound = {discussionMode:"tree"};
						}
						...
						doStuffOnCollection(
							...
							function(err, collection){
								collection.findOne(
									function(err, viewModeObjectFound){
										if(viewModeObjectFound==null){
											viewModeObjectFound=={viewMode:"discussion"};
										}
										
										res.json({discussionModeObject:discussionModeObjectFound, viewModeObject:viewModeObjectFound});
									}
								);
							}
						);
...
function saveCommentAndEmitUpdate(newComment, parentIdString){
	var parentId = stringToHex(parentIdString);
												...
												function(err2, records2)
												{
													collection.find().toArray(
														function(err, results){
															...
															io.sockets.emit("updateChannel", results, record);
														}
													);
												}
											);
		...
		function(err, collection){
			collection.find({sessionId:sessionId}).toArray(
				function(err, results){
					...
					var numberOfVotesInSession = 0;
					for(var i = 0; i < numberOfUsers; i++){
						//var indexOfUser = i;
						var theFunction = function(indexOfUser, userName){
							console.log("Finding votes for user " + userName + ", index " + indexOfUser);
							collection.find({sessionId:sessionId, userName:userName}).toArray(
								function(err, results){
									var voteScoresForUser = {};	
									voteScores.push(voteScoresForUser);
									
									if(results.length > numberOfVotesInSession){
										numberOfVotesInSession = results.length;
									}
									console.log("Got votes for a user in this session, indexOfUser is " + indexOfUser + "!");
									for(var i = 0; i < results.length; i++){
										var vote = results[i];
									...
									if(indexOfUser >= numberOfUsers - 1){
										console.log("voteScores is " + JSON.stringify(voteScores));
										...
										console.log("numberOfVotesInSession is " + numberOfVotesInSession);
...
io.sockets.on(
	...
	function (socket) {
		socket.on(
			"changeDiscussionMode",
			function(newDiscussionModeObject){
				doStuffOnCollection(
					"discussionMode",
					function(err, collection){
						collection.findOne(
							function(err, itemFound){
								if(itemFound==null){
									itemFound = newDiscussionModeObject;
								}
								else{
									itemFound.discussionMode = newDiscussionModeObject.discussionMode;
								}
								collection.save(
									itemFound,
									function(err, itemSavedOrOne){
						...
						collection.save(
							...
							function(err, itemSaved){
								...
								doStuffOnCollection(
									...
									function(err, collection){
										collection.save(
											{commentText:"", sessionId:currentSessionIdToSave, date:new Date().getTime()},
											function(err, itemSaved){
												for(var i = 0; i < alternatives.length; i++){
													saveCommentAndEmitUpdate({commentText:alternatives[i], sessionId:currentSessionIdToSave, date:new Date().getTime()}, itemSaved._id.toHexString());
												}
											}
										);
									}
													...
													function(err, collection){
														...
														collection.save(
															...
															function(err, itemSaved){
																...
																console.log("currentSession just before saving = " + JSON.stringify(currentSession));
																sessionsCollection.save(
																	...
																	function(err, itemSaved){
																		...
																		console.log("itemSaved is " + JSON.stringify(itemSaved) + ", err is " + err);
